...
| 2026-07-04 | Side swipe switches exercise in place, not by re-navigation | On device feedback showed swipe re-navigation rebuilt the camera and gesture detector, dropping the debounce and causing double triggers and a frozen preview. Switching in place keeps the camera and detector alive |
| 2026-07-04 | Gesture cooldown raised to 3 seconds with on screen confirmation | On device feedback showed gestures firing twice or feeling uncontrollable. A 3 second global cooldown plus a brief gesture feedback pill make control deliberate and legible |
| 2026-07-04 | Swipe requires a shoulder height, mostly horizontal sweep | On device feedback showed exercises changing accidentally. A swipe now only counts when the wrist is near shoulder height and the motion is predominantly horizontal, so squats (hands low) and jumping jacks (hands overhead or moving vertically) no longer trigger it |
| 2026-07-04 | A session records one ExerciseSet per exercise performed | On device feedback showed switching exercises forgot earlier reps. The session now accumulates each exercise segment, keeps a running on screen session total, sums reps and calories, and saves a set per exercise (schema allows many sets per session). The session is tagged with the exercise that contributed the most reps |